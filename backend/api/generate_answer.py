...
from api.models import Chunk, Conversation, Document as DocumentRecord, Message
from api.history_aware import ConversationTurn, prepare_history_aware_query
...
def load_chunks_from_database(project_id) -> list[IndexedChunk]:
    db = SessionLocal()
    try:
        query = db.query(Chunk, DocumentRecord.file_name).join(DocumentRecord, Chunk.document_id == DocumentRecord.document_id)
        query = query.filter(DocumentRecord.project_id == project_id)
        chunk_rows = query.order_by(Chunk.created_at.asc(), Chunk.chunk_id.asc()).all()
...
def build_conversation_history_block(conversation_history: Sequence[ConversationTurn]) -> str:
...
if __name__ == "__main__":
    print("generate_answer.py is now project-scoped and should be called through the API.")